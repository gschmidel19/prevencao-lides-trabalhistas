...
@st.cache_data
def carregar_dados():
    return pd.read_csv('dados/lides_trabalhistas_simulado.csv', parse_dates=['data_ajuizamento'])
...
if aba == 'Visão Geral':
    ...
    fig_estado = px.histogram(
        ...
        barmode='group', 
        title='Ações por Estado e Resultado'
    )
    ...
    fig_cargo = px.histogram(
        ...
        color='resultado_final', 
        barmode='group', 
        title='Ações por Setor e Resultado'
    )
    st.plotly_chart(fig_cargo, use_container_width=True)

elif aba == 'Recomendações':
    st.subheader('🔍 Recomendações Preventivas')
    cargos_risco = df[df['resultado_final'] == 'Procedente']['setor'].value_counts().head(5)
    st.write('Setores com mais ações procedentes:')
    st.table(cargos_risco)

    st.markdown('''
    ### Sugestões:
    - Reforçar treinamentos para os setores acima.
